...
<section class="max-w-7xl mx-auto px-6 py-20 flex flex-col md:flex-row items-center gap-12">
    ...
    <div class="flex-1 relative">
        ...
        <div class="absolute -bottom-6 -left-6 bg-white/90 backdrop-blur p-6 rounded-2xl shadow-xl z-20 border border-white">
...
<section id="events" class="max-w-7xl mx-auto px-6 py-20">
    <div class="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="/" class="{{ !request('category') ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }} px-5 py-2.5 rounded-xl font-bold transition shadow-sm">
                Semua Kategori
            </a>
            @foreach($categories as $cat)
            <a href="/?category={{ $cat->slug }}" class="{{ request('category') == $cat->slug ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }} px-5 py-2.5 rounded-xl font-bold transition shadow-sm">
                {{ $cat->name }}
            </a>
            @endforeach
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div class="group bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-300 overflow-hidden flex flex-col">
            <div class="relative overflow-hidden aspect-[3/4]">
                ...
                @else
                    <img src="https://placehold.co/400x600?text=No+Poster" alt="{{ $event->title }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                @endif
            ...
            <div class="p-6 flex flex-col flex-1">
                ...
                <div class="mt-auto pt-4 border-t flex justify-between items-center">
                    ...
                    <a href="/detail-event" class="px-5 py-2 bg-indigo-50 text-indigo-600 rounded-xl font-bold hover:bg-indigo-600 hover:text-white transition">Lihat Detail</a>
                </div>
            </div>
        </div>
        @empty
        <div class="col-span-full text-center py-20 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
            <p class="text-slate-500 font-bold text-lg">Yah, belum ada event untuk kategori ini 😢</p>
        </div>
        @endforelse
    </div>
</section>
